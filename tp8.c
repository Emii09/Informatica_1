//Link del repositorio https://github.com/Emii09/Informatica_1/tree/main || El tp actual se encuentra en la rama "tp8"
...
void cargar_piezas(float *p, int n){
	...
	for (int i = 0; i < n; i++){
		...
		do{
			printf("\nIngrese el diametro de la pieza n°%d: ",i+1);
			scanf("%f",&diam);
				
				if (diam <= 0 )
					printf("\nERROR, ingrese un diametro valido.\n");
...
float *reservar_memoria(int n){
	...
	if (p == NULL){
		...
		printf("\nERROR, NO SE PUDO RESERVAR MEMORIA, FINALIZANDO EL PROGRAMA\n");
